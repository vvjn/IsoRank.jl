...

{
    ...
    "category": "section",
    "text": "Assuming we have a matrix of prior node similarities, we can calculate the IsoRank matrix while incorporating external information. We treat the the node similarities as the personalization vector in PageRank. Here, b is a matrix of node similarities. Here, we equally weigh topological node similarity and prior node similarity by setting the alpha variable to 0.5. alpha must lie between 0.0 and 1.0. To give more weight to topological node similarity, increase the alpha variable up to 1.0.b = rand(size(G1,1), size(G2,1))\nb[sub2ind(size(b), 1:length(f), 1:length(f))] = 1.0\n\nR = isorank(G1, G2, 0.5, b)"
},
