...
function test2()
    ...
    R,res,L = isorank(G, G, 0.85, ones(2,2), maxiter=20, tol=1e-5, details=true,verbose=false)
...
function test3()
    G = sparse([1,2,3,1,3,2],[2,1,1,3,2,3],1,3,3)
    R,res,L = isorank(G, G, 0.85, ones(size(G,1),size(G,1)), maxiter=20, tol=1e-5, details=true,verbose=false)
